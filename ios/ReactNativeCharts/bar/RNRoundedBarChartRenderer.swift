...
open class RNRoundedBarChartRenderer: RNBarLineScatterCandleBubbleRenderer
{
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: BarChartDataSetProtocol, index: Int)
    {
        ...
        for j in buffer.indices
        {
            ...
            let e = dataSet.entryForIndex(j / stackSize) as? BarChartDataEntry
            let topIndex = e?.yValues?.lastIndex(where: { $0 > 0 }) ?? -1
